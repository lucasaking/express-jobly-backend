...
describe("POST /users/:username/jobs/:id", function () {
  test("works for users: apply for job", async function () {
  ...
  test("works for admins: apply for job", async function () {
    const result = await request(app).get("/jobs");
    const jobId = result.jobs[0].id;
    const resp = await request(app)
      .post(`/users/:u2/jobs/:${jobId}`)
  ...
  test("does not work for invalid jobId", async function () {
    const resp = await request(app)
  ...
  test("does not work for non admin to a different user", async function () {
    const result = await request(app).get("/jobs");
    const jobId = result.jobs[0].id;
    const resp = await request(app)
...
describe("GET /users/:username", function () {
  ...
  test("not found if user not found and not an admin", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth for non admin and other user", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        firstName: "New",
